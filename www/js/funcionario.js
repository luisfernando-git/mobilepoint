...
function findFuncionarios() {
    var url = 'http://localhost:9091/api/mobilepoint/funcionarios';
...
function saveFuncionario() {
    ...
    var urlPost = 'http://localhost:9091/api/mobilepoint/funcionarios';
    if (funcionario == undefined) {
        ...
        var urlPut = 'http://localhost:9091/api/mobilepoint/funcionarios/' + funcionario.id;
...
function deleteFuncionario(id) {
    var url = 'http://localhost:9091/api/mobilepoint/funcionarios/' + id;
    MobileUI.ajax.get(url).end(function(err, res) {
        if(err) {
            ...
            if (func.username == username) {
                ...
                MobileUI.ajax.delete(url).end(returnAPiDelete);
            }
        }
    });      
}

function returnAPiDelete(err) {
    if (err) {
        alert('Houve um problema na conexão, tente novamente!');
...
function updateFuncionario(id) {
    ...
    MobileUI.ajax.get(url).end(resultAjaxUpdate);
}

function resultAjaxUpdate(err, res){
    if(err) {
        alert('Houve um problema na conexão, tente novamente!');
    } else {
        if (res.body != null) {
